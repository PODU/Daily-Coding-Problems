...
func countLessEqual(A [][]int, x int) int {
	n, m := len(A), len(A[0])
	r, c, cnt := 0, m-1, 0
	for r < n && c >= 0 {
		if A[r][c] <= x {
...
func solve(A [][]int, i1, j1, i2, j2 int) int {
	n, m := len(A), len(A[0])
	smaller := countLessEqual(A, A[i1][j1])
	larger := n*m - countLessEqual(A, A[i2][j2])
