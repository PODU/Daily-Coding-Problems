...
public class Solution {
    static int intersectionArea(int tlx1, int tly1, int w1, int h1,
        ...
        int overlapW = Math.min(right1, right2) - Math.max(left1, left2);
        int overlapH = Math.min(top1, top2) - Math.max(bottom1, bottom2);
        return Math.max(0, overlapW) * Math.max(0, overlapH);
    }

    public static void main(String[] args) {
        System.out.println(intersectionArea(1, 4, 3, 3, 0, 5, 4, 3));
    }
}
