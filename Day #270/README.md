...
Given a weighted network with message-passing times on each edge, find the time for a message starting at node 0 to reach every node (the maximum shortest-path distance from node 0).
